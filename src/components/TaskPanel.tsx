...
export default function TaskPanel({ isOpen, onClose, mode, task }: TaskPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        {/* 头部 */}
        <div className="sticky top-0 z-10 flex items-center justify-between p-6 pb-4" style={{ background: 'var(--color-bg-surface-1)' }}>
          <div className="flex items-center gap-3">
            <span className="text-2xl">✏️</span>
            <h2 className="text-xl font-bold" style={{ color: 'var(--color-text-primary)', fontFamily: 'Quicksand, sans-serif' }}>
                ...
                {[1, 2, 3, 4, 5].map((p) => (
                  <button
                    ...
                    style={{
                      ...
                      border: priority === p ? `2px solid ${priorityColors[p]}` : '1px solid var(--color-border-light)',
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 flex items-center">
                ...
                />
                <span className="ml-3 text-sm" style={{ color: 'var(--color-text-muted)' }}>分钟</span>
              </div>
              <div className="flex gap-2">
                {quickDurations.map((mins) => (
                  <button
                    ...
                    style={{
                      background: estimatedMinutes === mins ? 'var(--color-blue)' : 'var(--color-bg-elevated)',
                      color: estimatedMinutes === mins ? '#fff' : 'var(--color-text-secondary)',
                      border: estimatedMinutes === mins ? '1px solid var(--color-blue)' : '1px solid var(--color-border-light)',
                    }}
                  >
        ...
        {/* 底部按钮 */}
        <div className="sticky bottom-0 flex items-center justify-end gap-3 p-6 pt-4" style={{ background: 'var(--color-bg-surface-1)' }}>
          <button
            onClick={onClose}
          ...
          <button
            ...
            style={{
              background: title.trim() ? 'linear-gradient(135deg, var(--color-blue) 0%, var(--color-purple) 100%)' : 'var(--color-border-light)',
              color: '#fff',
