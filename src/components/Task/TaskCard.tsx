  ...
  const progress =
    task.estimatedMinutes > 0
      ? Math.min(
          100,
          Math.round(
            ((task.timeLoggedMinutes || task.actualMinutes || 0) / task.estimatedMinutes) * 100
          )
        )
      : 0;
      ...
      <div className="flex items-start gap-3">
        ...
        {showMultiSelect && (
          <button
            ...
            style={{
              ...
              border: `2px solid ${selected ? 'var(--color-blue)' : 'var(--color-border-light)'}`,
            }}
          >
        ...
        >
          {task.status === 'completed' && (
            <span style={{ color: statusConfig.text }} className="text-sm font-bold">
              ✓
            </span>
          )}
        </button>
        ...
        <div className="flex-1 min-w-0">
          ...
          {task.firstStep && task.status !== 'completed' && (
            <div className="mt-2 p-2 rounded-lg" style={{ background: 'var(--color-blue)10' }}>
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            ...
            {task.estimatedMinutes > 0 &&
              task.status !== 'completed' &&
              !(task.timeLoggedMinutes || task.actualMinutes) && (
                <span
                  className="text-xs flex items-center gap-1"
                  style={{ color: 'var(--color-text-muted)' }}
                >
