...
export default function Dashboard() {
  ...
  // 计算今日各分类时间分布
  const categoryTimeMap = todayActivities.reduce(
    (acc, a) => {
  ...
  const todayTasks = tasks.filter((t) => {
    const taskDate = t.dueDate || t.createdAt?.slice(0, 10);
  ...
  if (todayMinutes === 0 && tasks.length === 0) {
    return (
      <div
        className="min-h-screen px-8 py-8 flex items-center justify-center"
        style={{ background: 'var(--color-bg-base)' }}
      >
  ...
  return (
    <>
      {/* Guardian Modals */}
      <MorningRitual isOpen={showMorningRitual} onComplete={() => setShowMorningRitual(false)} />
        ...
        <div className="mb-5 transition-all duration-200">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className="text-xs font-medium transition-all hover:opacity-80 flex items-center gap-1"
          ...
          <div className="grid grid-cols-3 gap-3">
            {/* 今日专注卡片 */}
            <div
              className="p-5 rounded-2xl transition-all duration-200 hover:shadow-md cursor-pointer"
              onClick={() => navigate('/analytics')}
              style={{
                ...
                <div>
                  <div className="flex items-center gap-2.5 mb-3">
                    ...
                    >
                      <Sparkles
                        size={20}
                        style={{ color: isFocusing ? 'var(--color-green)' : 'var(--color-blue)' }}
                      />
                    </div>
                    <span
              ...
              <div className="flex items-center justify-between">
                <div>
                    ...
                    <div
                      ...
                      style={{
                        background:
                          goalProgress >= 100
                            ? 'var(--color-green-soft)'
                            : 'var(--color-lemon-soft)',
                      }}
                    >
                      <TrendingUp
                        size={20}
                        style={{
                          color: goalProgress >= 100 ? 'var(--color-green)' : 'var(--color-lemon)',
                        }}
                      />
                    </div>
                    <span
                  ...
                  <div className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                    {goalProgress >= 100
                      ? '🎉 目标达成'
                      : `还需 ${Math.max(0, dailyGoalMinutes - todayMinutes)} 分钟`}
                  </div>
                </div>
                {/* 小圆环进度指示器 */}
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                    {pendingTasks > 0 ? `${pendingTasks} 个待完成` : '✨ 今日已完成'}
                  </div>
                </div>
                {/* 已完成任务头像堆叠预览 */}
                {completedTasks > 0 && (
        ...
        {isFocusing && (
          <div className="mb-4 text-center">
            <p
              ...
              style={{
                background:
                  'linear-gradient(135deg, var(--color-green-soft) 0%, var(--color-blue-soft) 100%)',
          ...
          <div className="relative">
            ...
            <div className="flex justify-between mb-1">
              {[0, 3, 6, 9, 12, 15, 18, 21].map((h) => (
                <span key={h} className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                  {h}:00
                </span>
              ))}
            </div>
