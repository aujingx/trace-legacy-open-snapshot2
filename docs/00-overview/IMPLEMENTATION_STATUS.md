...
> Trace 通过三个层次帮助用户：
>
> 1. **自动记录层**：静默追踪时间使用情况，让用户知道时间都花在哪里
> 2. **任务管理 × 时间追踪 融合层**：
>    - 在任务页面做计划（设预估时间、优先级）
>    - 在时间线页面做回顾（给时间块关联任务、查看实际耗时）
>    - 在 Dashboard 做启动（一键开始任务的专注、查看计划 vs 实际的对比）
>    - 所有数据自动同步、原生融合，不需要用户手动维护
> 3. **AI 引导层**：通过「隐形 Agent」潜移默化地帮助用户优化行为习惯，引导更高效的学习、工作和生活
>
> **核心设计原则**：不做评判，不制造焦虑，只做温和的引导和客观的呈现。
